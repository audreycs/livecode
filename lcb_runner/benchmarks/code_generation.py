...
def load_code_generation_dataset(release_version="release_v6", start_date=None, end_date=None) -> list[CodeGenerationProblem]:
    ...
    question_id_list = sorted([p.question_id for p in dataset])
...
def load_code_generation_dataset_not_fast(release_version="release_v6") -> list[CodeGenerationProblem]:
    dataset = load_dataset("livecodebench/code_generation", version_tag=release_version, split="test")
    dataset = dataset.shuffle(seed=42).select(range(100))
    dataset = [CodeGenerationProblem(**p) for p in dataset]  # type: ignore
    print(f"Loaded {len(dataset)} problems")
    # print question id and question title
    for p in dataset:
        print(f"{p.question_id}")
    return dataset
